	...
	public float getTotalCost() {
		...
		if (orderLines!= null) {
			...
			while (iter.hasNext()) {
				...
				sumWeight += ol.getProductWeight()*ol.getProductQuantity();
				System.out.print(ol.getProductQuantity());
			}
		}
		if(res>=500000) {
			System.out.print("Mien phi");
			return res;
		}
		else {
			float costDriver = 0;
			...
			if(address.equals("hn")||address.equals("hà nội")||address.equals("hcm")||address.equals("hồ chí minh")) {
				System.out.println("jfjfaj");
				if(sumWeight>3) {					costDriver = (float) (22000+Math.ceil(((sumWeight-3)*10)/5)*2500);
				}else {					costDriver =22000;
				}
			}
			else {
				System.out.println("nnnnj");
				if(sumWeight>0.5) {
					costDriver = (float) (30000+Math.ceil(((sumWeight-0.5)*10)/5)*2500);
				}else {
					costDriver = 30000;
				}
			}
			System.out.println(res+costDriver);
			return res + costDriver;
		}
//		return res;
	}
